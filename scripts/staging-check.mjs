...
const endpoint =
  process.env.COMMUNITY_WATCH_API_URL || 'https://server.matters.icu/graphql'
...
const run = async () => {
  ...
  const missingMutations = requiredMutations.filter(
  ...
  console.log(
    `Public record count: ${records.communityWatchActions.totalCount}`
  )
  ...
  if (token) {
    ...
    console.log(
      `Viewer: ${viewer.viewer.displayName} (@${viewer.viewer.userName})`
    )
    ...
    console.log(
      `Viewer communityWatch: ${flags.includes('communityWatch') ? 'yes' : 'no'}`
    )
  }
